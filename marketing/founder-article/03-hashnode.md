...
Title to use: "I built an AI journal because my brain wouldn't switch off"
-->
...
Most journaling apps assume you know what to write about. They give you prompts like "what are you grateful for today?" That's not the problem at 2:47 AM. The problem is that your brain has fourteen unresolved threads from the day and it's trying to file them all at once. Asking it to be grateful is like asking a fire alarm to consider its tone.
...
I needed someone to look at the mess and say: *here, this is what you're actually carrying.*
...
The technical heart of NexoMind is a single design choice: the model is forced to return a typed, four-field JSON object instead of free-form text.

That sounds small. It changed everything. Open-ended chat completions kept producing wandering, comforting answers — exactly the shape of journaling I was trying to escape. A typed schema forces the model to *commit* to naming the parts instead of *consoling* the writer.

The four-field constraint became the design constraint became the brand. NexoMind doesn't talk to you. It hands the thought back to you in pieces small enough to hold.

## The hard part

> [PERSONALIZE] — Keep the shape ("I almost added too much, here's what I cut") or swap in your actual hard part if it was different.
